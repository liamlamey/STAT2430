...
--> Distribution of bill length and bill depth in three noticable groups, do not see the color or what species of penguins each dot represents.
...
q2.2
--> Now can see that the three groups are different species, with each dot representing Adelie, Chinstrap or Gentoo
...
penguins |> 
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, color=species,)) + 
  geom_point()+ labs(title = "Scatter Plot of Bill Length vs. Bill Depth ",
       x = "Bill Length (mm)",
       y = "Bill Depth (mm)")
```
...
penguins |> 
 ...
 geom_point()+ labs(title = "Scatter Plot of Bill Length vs. Bill Depth ",
       x = "Bill Length (mm)",
       y = "Bill Depth (mm)",
       size="Body Mass (g)")+scale_color_viridis_d()
```
